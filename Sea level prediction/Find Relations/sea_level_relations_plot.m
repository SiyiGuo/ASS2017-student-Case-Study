...
cycle_length=1;%12 can be changed to other length of period
                %should be in the factor of 300
max_matrix=zeros(20,300/cycle_length);

for i = 1:20 %from block 19 to block 20
    max=-99999;
...
for i=1:1
    ...
    for j=1:(300 / cycle_length) %300 month / 12month(cycle_length) = 25
        put_0 = 'noo';
        for a = 1:length(target_block)
            if max_matrix(target_block(a),j) == -99999
                put_0 = 'yes';
            end
        end
        ...
        if put_0 == 'yes'
            ...
            data = max_matrix(3,j);
            mean = (max_matrix(2,j) + max_matrix(4,j))/2;
            data = data - mean;
            v = [v data];
        end
    end
